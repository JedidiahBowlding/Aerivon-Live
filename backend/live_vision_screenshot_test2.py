...
async def main() -> None:
    ...
    async with client.aio.live.connect(model=MODEL, config=config) as stream:
        parts = [
            types.Part.from_text(text="What domain is shown in this screenshot? Reply with the domain only."),
            types.Part.from_bytes(data=img, mime_type="image/png"),
        ]
        await stream.send_client_content(turns=types.Content(role="user", parts=parts), turn_complete=True)
        ...
        async for msg in stream.receive():
            ...
            if sc is not None and getattr(sc, "turn_complete", None) is True:
                break
            if asyncio.get_running_loop().time() - start > 10.0:
                break
